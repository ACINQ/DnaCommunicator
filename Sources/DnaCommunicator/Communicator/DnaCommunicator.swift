...
public class DnaCommunicator {
	...
	public internal(set) var activeKeyNumber: KeySpecifier = .KEY_0
	
	var activeTransactionIdentifier: [UInt8] = [0,0,0,0]
	var commandCounter: Int = 0
	var sessionEncryptionMode: EncryptionMode?

	public init(tag: NFCISO7816Tag, logger: @escaping (String) -> Void) {
		self.tag = tag
		self.logger = logger
	}
	...
	func logBytes(_ name: String, _ data: [UInt8]) {
		logger(name + ": " + data.toHex())
	}
	...
	func makeErrorIfNotExpectedStatus(_ result: NxpCommandResult) -> Error? {
		if result.statusMajor != 0x91 || (result.statusMinor != 0x00 && result.statusMinor != 0xaf) {
			...
			return Utils.makeError(102, "Unexpected status: \(major) / \(minor)")
		} else {
			return nil
		}
	}
	...
	func isoTransceive(
		...
		let apdu = NFCISO7816APDU(data: packet.toData())
		logBytes("Outbound", packet)

		guard let apdu else {
			log("APDU Failure: Attempt")
			return .failure(Utils.makeError(100, "APDU Failure"))
		}
		
		do {
			let (data, sw1, sw2) = try await tag.sendCommand(apdu: apdu)
			
			let bytes: [UInt8] = data.toByteArray()
			logBytes("Inbound", bytes + [sw1] + [sw2])
	...
	public func nxpMacCommand(
		...
		case .success(let result):
			...
			if returnMacData.elementsEqual(macBytes) {
				...
				log("Invalid MAC! (\(returnMacData)) / (\(macBytes)")
				return .failure(Utils.makeError(101, "Invalid MAC"))
			}
		}
	}
	...
	public func nxpEncryptedCommand(
		...
		let data = data ?? [UInt8]()
		logBytes("Unencryped outgoing data", data)
		...
		case .success(let result):
			...
			logBytes("Unencrypted incoming data", decryptedResultData)
