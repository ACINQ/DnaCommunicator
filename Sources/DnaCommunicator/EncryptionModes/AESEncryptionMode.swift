...
public class AESEncryptionMode : EncryptionMode {
	...
	init(
		...
		communicator.logBytes("Session Enc Key", sessionEncryptionKey)
		communicator.logBytes("Session Mac Key", sessionMacKey)
	}
	...
	func generateIv(purpose: [UInt8]) -> [UInt8] {
		let iv = Utils.simpleAesEncrypt(key: sessionEncryptionKey, data: generateIvInput(purpose: purpose))
		communicator?.logBytes("IV", iv)
		return iv
	}
	...
	func generateIvInput(purpose:[UInt8]) -> [UInt8] {
		...
		communicator?.logBytes("IV Input", ivInput)
		return ivInput
	}
    
	func encryptData(message: [UInt8]) -> [UInt8] {
		return Utils.simpleAesEncrypt(
			key: sessionEncryptionKey,
			data: Utils.messageWithPadding(message),
			iv: generateIv(purpose: AESEncryptionMode.encryptionPurpose)
		)
	}

	func decryptData(message: [UInt8]) -> [UInt8] {
		return Utils.simpleAesDecrypt(
			key: sessionEncryptionKey,
			data: message,
	...
	func generateMac(message: [UInt8]) -> [UInt8] {
		let fullMac = Utils.simpleCMAC(key: sessionMacKey, data: message)
		return Utils.evensOnly(fullMac)
	}
	...
	static func generateAESSessionKey(
		...
		return Utils.simpleCMAC(key: key, data: sessionVector)
	}
